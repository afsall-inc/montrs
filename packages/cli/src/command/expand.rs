...
pub async fn run(path: String) -> Result<()> {
    ...
    let file: File = parse_str(&content).map_err(|e| anyhow!("Failed to parse sketch file: {}", e))?;
    ...
    if !expanded {
        return Err(anyhow!("Could not determine component type in sketch. Ensure it implements Plate, Route, or AppConfig."));
    }
...
fn expand_plate(file: &File, content: &str) -> Result<()> {
    ...
    let plate_struct: &syn::ItemStruct = file.items.iter().find_map(|item| {
...
fn expand_route(file: &File, content: &str) -> Result<()> {
    ...
    let route_struct = file.items.iter().find_map(|item| {
        if let Item::Struct(s) = item {
    ...
    let target_dir = PathBuf::from("src/plates").join(&plate_name).join("routes");
...
fn register_module<P: AsRef<Path>>(mod_rs_path: P, mod_name: &str) -> Result<()> {
