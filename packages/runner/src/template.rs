...
pub fn render_task(task: &mut Task, extra_vars: &HashMap<String, String>) -> anyhow::Result<()> {
    ...
    ctx.insert("config_root", &task.config_root.as_ref().map(|p| p.to_string_lossy().to_string()));
    ...
    if let Some(ref dir) = task.dir {
        task.dir = Some(tera.render_str(dir, &ctx).unwrap_or_else(|_| dir.clone()));
    }

    // Render env values
    let mut rendered_env = std::collections::HashMap::new();
    for (key, val) in &task.env {
        let rendered = tera.render_str(val, &ctx).unwrap_or_else(|_| val.clone());
...
fn create_tera() -> tera::Tera {
    ...
    tera.register_function("env", |args: &HashMap<String, serde_json::Value>| {
        let key = args
            .get("var")
        ...
        let val = std::env::var(key).ok().or_else(|| default.map(|s| s.to_string()));
        Ok(serde_json::Value::String(val.unwrap_or_default()))
    });

    tera.register_function("cwd", |_args: &HashMap<String, serde_json::Value>| {
    ...
    tera.register_function("throw", |_: &HashMap<String, serde_json::Value>| {
