...
impl VideoComposition {
    ...
    pub fn render(&self, output: &Path) -> Result<(), VideoError> {
        ...
        let temp_dir = tempfile::tempdir().map_err(|e| VideoError::Io(e.to_string()))?;
        let temp_path = temp_dir.path();
        for frame in &self.frames {
            let png_path = temp_path.join(format!("frame_{:04}.png", frame.index));
        ...
        let status = Command::new("ffmpeg")
            ...
            .status()
            .map_err(|e| VideoError::Ffmpeg(format!("FFmpeg not found: {}", e)))?;
        if !status.success() {
...
fn render_svg_to_png(svg: &str, width: u32, height: u32, output: &Path) -> Result<(), VideoError> {
    #[cfg(feature = "video")]
    {
    ...
    #[cfg(not(feature = "video"))]
    {
        ...
        Err(VideoError::Svg("Video feature not enabled. Use --features video".into()))
    }
}
