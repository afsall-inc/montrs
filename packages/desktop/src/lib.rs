...
/// Launch a native desktop application using the CompositorRenderer.
///
/// Creates a `CompositorRenderer` wrapping the tiny-skia backend (CPU,
/// portable), providing the full compositor layer stack. For GPU-accelerated
/// rendering, use `run_native` with a wgpu backend directly.
#[cfg(feature = "native")]
pub fn run_native_compositor(
    title: &str,
    width: u32,
    height: u32,
) -> Result<(), DesktopError> {
    ...
    let backend: Box<dyn montrs_renderer::Renderer> = Box::new(
        montrs_renderer::backend::tiny_skia::SkiaRenderer::new(
            width.max(1),
            height.max(1),
        ),
    );

    let mut compositor_renderer =
        montrs_renderer::CompositorRenderer::new(backend);
        ...
        *control_flow = ControlFlow::Poll;
        ...
        match event {
            ...
            Event::WindowEvent {
                ...
                compositor_renderer.begin(&viewport);
                // Application code adds layers, quads, paths here
                compositor_renderer.finish();
                window.request_redraw();
            }
            _ => {}
        }
