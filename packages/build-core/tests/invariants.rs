...
fn test_build_step_enum_values() {
    let steps = [BuildStep::Server, BuildStep::Frontend, BuildStep::Tailwind, BuildStep::Assets, BuildStep::IndexHtml];
...
fn test_build_pipeline_trait_is_object_safe() {
    ...
    impl BuildPipeline for MockPipeline {
        ...
        fn metadata(&self) -> &montrs_metadata::MontrsMetadata { unimplemented!() }
...
fn test_find_workspace_target_dir_default() {
    let result = find_workspace_target_dir(std::path::Path::new("/nonexistent"));
    assert!(result.is_ok());
}