use crate::{
    prdoc::{Audience, BumpLevel, CrateChange, PrDoc, PrDocStatus},
    prdoc_analyzer::{ChangeCategory, DiffAnalysis, PrContext},
    prdoc_embed,
};
...
pub fn generate_prdoc(
    ...
    let commit_categories = context
        .map(|c| {
            let rule_based =
                crate::prdoc_analyzer::analyze_commits(&c.commit_messages);
            let embed_based: Vec<ChangeCategory> = c
                .commit_messages
                .iter()
                .filter_map(|m| prdoc_embed::classify_by_embedding(m))
                .collect();
...
pub fn render_prdoc(prdoc: &PrDoc, analysis: &DiffAnalysis) -> String {
    ...
    if !analysis.summary_hints.is_empty() {
        for hint in &analysis.summary_hints {
            out.push_str(&format!("- {}\n", hint));
        }
        out.push('\n');
    } else {
