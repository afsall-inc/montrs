...
where
    ...
    S: MachineState<Context = C> + Clone + 'static,
{
    ...
    pub fn send(&mut self, event: &E) -> StateResult<&S> {
        ...
        for guard in &transition.guards {
            if !guard.check(&self.context, event) {
        ...
        self.current =
            S::construct(transition.to.clone(), self.context.clone());
        Ok(&self.current)
    }
}
...
where
    C: Clone + Default + PartialEq + Send + Sync + 'static,
    E: Clone + PartialEq + fmt::Debug + 'static,
{
    fn default() -> Self {
        Self::new()
    ...
    E: Clone + PartialEq + fmt::Debug + 'static,
{
    ...
    pub fn transition(
        ...
        if let Some(node) = self.states.iter_mut().find(|n| n.name == from) {
            node.transitions
                .push(Transition::new(from.clone(), event, to));
        } else {
...
pub struct FunctionAction<C, E, F> {
    f: F,
    name: String,
    _phantom: std::marker::PhantomData<fn(C, E)>,
}
...
where
    ...
    E: fmt::Debug,
{
    fn execute(&self, _context: &mut C, event: &E) {
        log::info!(target: "montrs_state", "action: {} event: {:?}", self.name, event);
    }
    fn name(&self) -> &str {
        &self.name
    }
...
pub struct AssignAction<C, E, T, F> {
    ...
    f: F,
    name: String,
    _phantom: std::marker::PhantomData<fn(C, E, T)>,
}
impl<C, E, T, F> AssignAction<C, E, T, F>
where
    F: Fn(&C, &E) -> T + Send + Sync + 'static,
    C: Send + Sync,
    E: Send + Sync,
    T: Send + Sync + 'static,
{
...
impl<C, E> Guard<C, E> for OrGuard<C, E> {
    fn check(&self, context: &C, event: &E) -> bool {
        self.guards.iter().any(|g| g.check(context, event))
    }
    fn name(&self) -> &str {
        &self.name
    }
...
pub struct NotGuard<C, E> {
    inner: Box<dyn Guard<C, E>>,
    name: String,
}
impl<C, E> NotGuard<C, E> {
    pub fn new(name: impl Into<String>, inner: Box<dyn Guard<C, E>>) -> Self {
        Self {
            inner,
            name: name.into(),
        }
...
impl<C, E> Guard<C, E> for NotGuard<C, E> {
    fn check(&self, context: &C, event: &E) -> bool {
        !self.inner.check(context, event)
    }
    fn name(&self) -> &str {
        &self.name
    }
...
impl<C> HistoryTracker<C> {
    ...
    pub fn push(&mut self, entry: HistoryEntry<C>) {
        self.entries.push(entry);
        while self.entries.len() > self.limit {
            self.entries.remove(0);
        }
    }
    pub fn entries(&self) -> &[HistoryEntry<C>] {
        &self.entries
    }
...
pub struct MachineHistory<C> {
    tracker: HistoryTracker<C>,
}

impl<C> MachineHistory<C> {
    pub fn new(limit: usize) -> Self {
        Self {
            tracker: HistoryTracker::new(limit),
        }
    }
    pub fn push(&mut self, state: impl Into<String>, context: C) {
        self.tracker.push(HistoryEntry {
            state: state.into(),
            context,
...
impl MachineError {
    ...
    pub fn from_state_error(error: StateError) -> Self {
        Self {
...
mod tests {
    ...
    fn guard_rejects() {
        let guard =
            FunctionGuard::new("always_false", |_: &i32, _: &StringEvent| {
                false
            });
