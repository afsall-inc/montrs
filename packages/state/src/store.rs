...
impl<T: Store> Store for Arc<T> {
    type State = T::State;
    fn get(&self) -> Self::State { (**self).get() }
    fn set(&self, state: Self::State) { (**self).set(state) }
    fn update_boxed(&self, f: Box<dyn FnOnce(Self::State) -> Self::State + Send + Sync>) { (**self).update_boxed(f) }
    fn update<F>(&self, f: F) where F: FnOnce(Self::State) -> Self::State + Send + Sync + 'static { (**self).update(f) }
}
...
impl<T: Clone + PartialEq + Send + Sync + 'static> SimpleStore<T> {
    pub fn new(initial: T) -> Self { Self { state: Arc::new(RwLock::new(initial)) } }
}

impl<T: Clone + PartialEq + Send + Sync + 'static> Store for SimpleStore<T> {
    type State = T;
    fn get(&self) -> T { self.state.read().map(|s| s.clone()).unwrap_or_else(|_| panic!("lock poisoned")) }
    fn set(&self, state: T) { *self.state.write().unwrap_or_else(|_| panic!("lock poisoned")) = state; }
    fn update_boxed(&self, f: Box<dyn FnOnce(T) -> T + Send + Sync>) { let current = self.get(); self.set(f(current)); }
    fn update<F>(&self, f: F) where F: FnOnce(T) -> T + Send + Sync + 'static { self.update_boxed(Box::new(f)) }
}

impl<T> fmt::Debug for SimpleStore<T> where T: Clone + PartialEq + Send + Sync + 'static + fmt::Debug {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("SimpleStore").field("state", &self.get()).finish()
    }
}
...
impl<T: Clone + PartialEq + Send + Sync + 'static> StoreContext<T> {
    pub fn new(store: SimpleStore<T>) -> Self { Self { inner: store.state.clone() } }
    pub fn get(&self) -> T { self.inner.read().map(|s| s.clone()).unwrap_or_else(|_| panic!("lock poisoned")) }
    pub fn set(&self, state: T) { *self.inner.write().unwrap_or_else(|_| panic!("lock poisoned")) = state; }
    pub fn update<F>(&self, f: F) where F: FnOnce(T) -> T + Send + Sync + 'static {
        let current = self.get(); self.set(f(current));
    }
}
...
where T: Clone + PartialEq + Send + Sync + 'static, O: Clone + PartialEq + 'static {
    type Output = O;
    fn select(&self, state: &T) -> O { (self.selector)(state) }
}

pub trait Middleware<T: Store>: Send + Sync {
    fn on_get(&self, state: &T::State) -> T::State { state.clone() }
    fn on_set(&self, _old: &T::State, new: &T::State) -> T::State { new.clone() }
}
...
impl<T: Store> MiddlewareChain<T> {
    ...
    pub fn add(&mut self, middleware: impl Middleware<T> + 'static) { self.middlewares.push(Box::new(middleware)); }
    pub fn apply_get(&self, state: &T::State) -> T::State {
        let mut state = state.clone();
        for m in &self.middlewares { state = m.on_get(&state); }
        state
    }
    pub fn apply_set(&self, old: &T::State, new: &T::State) -> T::State {
        let mut state = new.clone();
        for m in &self.middlewares { state = m.on_set(old, &state); }
        state
    }
}
...
impl<T: Store> Middleware<T> for LoggerMiddleware where T::State: fmt::Debug {
...
where F: Fn(&T::State) -> bool + Send + Sync {
    pub fn new(validator: F) -> Self { Self { validator, _phantom: std::marker::PhantomData } }
}

impl<T: Store, F> Middleware<T> for ValidationMiddleware<T, F>
where F: Fn(&T::State) -> bool + Send + Sync {
    fn on_set(&self, _old: &T::State, new: &T::State) -> T::State {
        if (self.validator)(new) { new.clone() } else { _old.clone() }
    }
}
