...
use crate::machine::{Machine, MachineHistory, MachineState, StateMachine};
use crate::store::{SimpleStore, Store, StoreContext, StoreSlice};
use crate::time_travel::TimeTravel;
use leptos::prelude::*;

pub fn provide_store<T>(store: StoreContext<T>)
where T: Clone + PartialEq + Send + Sync + 'static { provide_context(store); }

pub fn use_store_value<T>() -> T
where T: Clone + PartialEq + Send + Sync + 'static { expect_context::<StoreContext<T>>().get() }
...
impl<T: Clone + PartialEq + Send + Sync + 'static> StoreActions<T> {
    pub fn set(&self, state: T) { self.store.set(state); }
    pub fn update<F>(&self, f: F) where F: FnOnce(T) -> T + Send + Sync + 'static { self.store.update(f); }
    pub fn reset(&self, initial: T) { self.store.set(initial); }
}
...
where T: Clone + PartialEq + Send + Sync + 'static {
    ...
    Effect::new(move |_| { set_signal.set(store.get()); });
    (signal, set_signal)
}
...
pub fn use_store_slice<S, Slice>(store: StoreContext<S>, selector: Slice) -> Memo<Slice::Output>
where S: Clone + PartialEq + Send + Sync + 'static, Slice: StoreSlice<SimpleStore<S>> + 'static, Slice::Output: Clone + PartialEq + 'static {
...
where M::State: Clone + 'static, M::Event: Clone + 'static, M::Context: Clone + 'static {
    ...
    MachineHandle { state, send, context }
}

pub fn use_machine_with_instance<M: StateMachine + 'static>(machine: M) -> (ReadSignal<M::State>, Callback<M::Event>)
where M::State: Clone + 'static, M::Event: Clone + 'static {
    let initial = M::initial();
    let (state, set_state) = signal(initial);
    let send = Callback::new(move |event: M::Event| {
        let current = state.get();
...
pub fn use_machine_history<M: StateMachine + 'static>() -> MachineHistory<M::Context>
where M::Context: Clone + 'static {
